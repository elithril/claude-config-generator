...
export const DEFAULT_HOOKS: HookEntry[] = [
  ...
  {
    id: "hook-log-permissions",
    event: "PermissionRequest",
    action: "command",
    command: "echo \"$(date): Permission requested\" >> .claude/permissions.log",
    description: "Logger les demandes de permission pour audit",
    enabled: false,
  },
];
...
export const DEFAULT_MCP_SERVERS: McpServer[] = [
  // === Popular ===
  {
    id: "mcp-github",
    name: "GitHub",
    description: "Gestion de repos, PRs, issues via l'API GitHub. Nécessite un Personal Access Token (PAT).",
    transport: "stdio",
    command: "npx",
  ...
  },
  {
    id: "mcp-postgres",
    name: "PostgreSQL",
    description: "Requêtes et exploration de BDD PostgreSQL. Modifie l'URL de connexion ci-dessous.",
    transport: "stdio",
    command: "npx",
  ...
  },
  {
    id: "mcp-filesystem",
    name: "Filesystem",
    description: "Accès contrôlé au système de fichiers local",
    transport: "stdio",
    command: "npx",
  ...
  },
  {
    id: "mcp-memory",
    name: "Memory",
    description: "Mémoire persistante entre sessions via un graphe de connaissances",
    transport: "stdio",
    command: "npx",
...
export function getDefaultConfig(): ClaudeConfig {
  return {
    ...
    permissions: {
      allow: [],
      ask: [],
      deny: ["Read(./.env)", "Read(./.env.*)", "Read(./secrets/**)", "Read(./**/*.pem)", "Read(./**/*.key)", "Read(./**/credentials.json)", "Read(./**/service-account.json)"],
    },
    enableHooks: false,
    enableMCP: false,
    enableRules: false,
